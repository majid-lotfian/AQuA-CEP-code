...
public class Producer implements Serializable {
    ...
    public static DataPair CalculateData(ProducerEventTypeMatch pet) {
        ...
        String newGT = GenerateGroundTruthData(Controller.queryOffset);


        System.out.println("query offset : "+Controller.queryOffset);
        if (Controller.DynamicLossRatePosition[Controller.queryOffset] == 1){
        ...
        if (Controller.DynamicLatencyPosition[Controller.queryOffset] == 1){
    ...
    public void producerStart(ProducerEventTypeMatch pet){
        ...
        try {
            ...
            while (pet.getQ().isActive() && !stop&& Controller.queryOffset < pet.getQ().getDuration()){


                //if (Controller.stopSendingData){
                ...
                if ((currentTime -pet.getQ().getStartTime()) % pet.getP().getSensingInterval() == 0){
                    ...
                    //stop = Controller.qma.QMACheck();
                    //System.out.println("stop producer : "+stop);

                    //System.out.println("Accuracy level : "+ pet.getP().getQmList().get(0).getMetricThreshold());

                    if(Controller.queryOffset == pet.getQ().getDuration()){
                        System.out.println("Query processed successfully");
                        Controller.terminateQuery(pet.getQ());
                    }
                }
                try{

                    Thread.sleep(pet.getP().getSensingInterval()*1000);
    ...
    public static String GenerateGroundTruthData(int i){
        ...
        String result = "";
        int sign = rand.nextInt(2); // 0 means positive and 1 means negative
        int change = rand.nextInt(2); // the random number whether 0 or 1, shows the change
        if(type.equals("Temperature")){
            double newTemp;
            if (start == ct){
                newTemp = baseTemp;
            }else{
                newTemp = previousGTTempData;
                if (sign==0){
                    newTemp += change;
                }
                if(sign == 1){
        ...
        if(type.equals("Smoke")){
            ...
            if (start == ct){
                ...
                newSmoke = previousGTSmokeData;

                if (sign==0){
                    if (previousGTSmokeData<97) {
                        newSmoke += 3 * change;
                    }
                }
                if(sign == 1){
                    if (previousGTSmokeData>3){
                        newSmoke -= 3 * change;
                    }
                }
    ...
    /*public static String setGroundTruth(ProducerEventTypeMatch pet){
        String r="";
        if (pet.getP().getType() == "Temperature"){
            r = String.valueOf(previousGTTempData);
        }
        if (pet.getP().getType() == "Smoke"){
            r = String.valueOf(previousGTSmokeData);
        }
        return r;
    }*/
