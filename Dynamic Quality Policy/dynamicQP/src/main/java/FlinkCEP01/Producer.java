...
public class Producer implements Serializable {
    ...
    public static Point[] CalculateData(ProducerEventTypeMatch pet) {
        ...
        double randomXAccuracy = random.nextDouble()*pAccuracy;
        double randomYAccuracy = random.nextDouble()*pAccuracy;
        //int randomLoss = random.nextInt(100);


        //DataPair dp = new DataPair();
        Point newgt = GenerateGroundTruthData(pet.getQ().getQueryOffset());
        Point newGT = new Point((int)newgt.getX()*10,(int)newgt.getY()*10);
        pair[0] = newGT;
        System.out.println("query "+pet.getQ().getID()+ " offset : "+pet.getQ().getQueryOffset());
        int XCoords = (int) (newGT.getX()-(randomXAccuracy));
        int YCoords = (int) (newGT.getY()-(randomYAccuracy));
        if (XCoords < 0)
            XCoords = 0;
        else if (XCoords > 380) {
            XCoords = 380;
        ...
        Point producerGeneratedPoint = new Point(XCoords, YCoords);
    ...
    public void producerStart(ProducerEventTypeMatch pet){
        ...
        try {
            ...
            while (pet.getQ().isActive() && !stop&& pet.getQ().getQueryOffset() < pet.getQ().getDuration()){
                ...
                Location whileLocation = new Location();
                if ((currentTime -pet.getQ().getStartTime()) % pet.getP().getSensingInterval() == 0){
                    ...
                    Point[] newP = Producer.CalculateData(pet);
                    pet.getQ().getEtList().get(0).setPreviousLocation(pet.getQ().getEtList().get(0).getCurrentLocation());
                    pet.getQ().getEtList().get(0).setCurrentLocation(new Location(newP[1].getX(),newP[1].getY()));
                    data.add(new AttributeValue("ProducerID", pet.getP().getID()));
                    data.add(new AttributeValue("Type", pet.getP().getType()));
                    data.add(new AttributeValue("Value", (int)newP[1].getX()+":"+(int)newP[1].getY()));
                    data.add(new AttributeValue("GTValue", (int)newP[0].getX()+":"+(int)newP[0].getY()));
                    ...
                    data.add(new AttributeValue("accuracy", "" + pet.getP().getAccuracy()));
    ...
    public static Point GenerateGroundTruthData(int i){



        return Controller.routeP1.get(i);
